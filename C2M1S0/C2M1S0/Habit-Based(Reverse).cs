...
namespace C2M1S0
{
    public class HabitBasedReverse : MatchSrategy
    {
        public void MatchMethod(Individual me, List<Individual> preys)
        {
            int habitsMatchMinTimes = int.MaxValue;
            ...
            foreach (Individual prey in preys)
            {
                ...
                if (me.Id != prey.Id)
                {
                    ...
                    if (habitsMatchMinTimes > habitsMatchTimes)
                    {
                        habitsMatchMinTimes = habitsMatchTimes;
                        match = prey;
                    }
                    else if (habitsMatchMinTimes == habitsMatchTimes)
                    {
            ...
            Console.WriteLine($"興趣交集次數:{habitsMatchMinTimes}");
        }
    }
}
